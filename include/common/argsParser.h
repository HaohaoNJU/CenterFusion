...
struct Args
{
    ...
    bool useILoop{false};
    std::string detectionOnnxPath, fusionOnnxPath, detectionEnginePath, fusionEnginePath, savePath, filePath;
    float scoreThre{0.2};
    float nmsThre{0.7};
...
inline bool parseArgs(Args& args, int32_t argc, char* argv[])
{
    while (1)
    {
        ...
        static struct option long_options[] = {
            ...
            {"useDLACore", required_argument, 0, 'c'},
            {"batch", required_argument, 0, 'b'},
            
            {"scoreThre", required_argument, 0, 'm'},
            {"nmsThre", required_argument, 0, 'n'},


            {"detectionOnnxPath", required_argument, 0, 'u'},
            {"fusionOnnxPath", required_argument, 0, 'v'},
            {"detectionEnginePath", required_argument, 0, 'w'},
            {"fusionEnginePath", required_argument, 0, 'x'},
        ...
        if (optarg)
            {
                args.fusionOnnxPath = optarg;
            }
            break;
        case 'w':
        if (optarg)
            {
                args.detectionEnginePath = optarg;
            }
            break;
        case 'x':
        if (optarg)
            {
                args.fusionEnginePath = optarg;
            }
            break;
        case 'y':
        if (optarg)
        ...
        if (optarg)
            {
                args.scoreThre = atof(optarg);
            }
            break;
        case 'n':
        if (optarg)
            {
                args.nmsThre = atof(optarg);
            }
            break;
        ///////////////////////////////////////////////////////End For CenterPoint/////////////////////////////////////////////////////////
