...
#DATA_PATH = "/mnt/cifs/nuscenes/nuScenes/"
DATA_PATH = '/mnt/nas/nuscenes/nuScenes/'
  ...
  for split in SPLITS:
    ...
    for sample in nusc.sample:
      ...
      if sample['prev'] == '':
        ...
        track_ids = {}
      ##########################################################################################
      # radar_channel = 'RADAR_FRONT_LEFT'
        ...
        if sensor_name in USED_SENSOR:
          ...
          for radar_channel in RADARS_FOR_CAMERA[sensor_name]:
            try:
              ...
              print('CUT here ', radar_channel, sensor_name)
              radar_pcs, _ = RadarPointCloud.from_file_multisweep_pdb(nusc, 
                sample, radar_channel, sensor_name, NUM_SWEEPS)
              # After debug, run again ! 
              radar_pcs, _ = RadarPointCloud.from_file_multisweep(nusc, 
                sample, radar_channel, sensor_name, NUM_SWEEPS)
