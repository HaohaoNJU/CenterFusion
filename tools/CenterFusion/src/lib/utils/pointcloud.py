...
class RadarPointCloudWithVelocity(RadarPointCloud):
    ...
    def from_file_multisweep(cls,
        ...
        for _ in range(nsweeps):
            ...
            file_name = osp.join(nusc.dataroot, current_sd_rec['filename'])
            # pdb.set_trace()
            current_pc = cls.from_file(file_name)
            ######################################################################
    ...
    @classmethod
    def from_file_multisweep_pdb(cls,
                             nusc: 'NuScenes',
                             sample_rec: Dict,
                             chan: str,
                             ref_chan: str,
        ...
        for idx_ in range(nsweeps):
            # Load up the pointcloud and remove points close to the sensor.
            file_name = osp.join(nusc.dataroot, current_sd_rec['filename'])
            print(idx_,file_name)
            try:
              current_pc = cls.from_file(file_name)
            except:
              pdb.set_trace()
...
def generate_pc_hm(output, pc_dep, calib, opt):
      ...
      # every sample choose topK scores .  
      ...
      for i, [pc_dep_b, bboxes_b, depth_b, dim_b, rot_b] in enumerate(zip(pc_dep, bboxes, dep, dims, rot)):
        ...
        for j, [bbox, depth, dim, alpha] in enumerate(zip(bboxes_b, depth_b, dim_b, alpha_b)):
          ...
          # get z-axis distance range of 3dboxs, computed from 8 coners
          dist_thresh = get_dist_thresh(calib, ct, dim, alpha)
          # dist_thresh is delta in paper 
...
def pc_dep_to_hm_torch(pc_hm, pc_dep, dep, bbox, dist_thresh, opt):
    ...
    bbox_int = torch.tensor([torch.floor(bbox[0]).int(), 
                         torch.floor(bbox[1]).int(), 
                         torch.ceil(bbox[2]).int(), 
                         torch.ceil(bbox[3]).int()], dtype=torch.int32)# format: xyxy
