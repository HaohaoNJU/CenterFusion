...
class Detector(object):
    ...
    def run(self, image_or_path_or_tensor, meta={}):
        ...
        if isinstance(image_or_path_or_tensor, np.ndarray):
            ...
            if pc_dep is not None:
                if self.opt.flip_test:
                    flipped = torch.flip(pc_dep, [3]) # left right flip
                    channel = self.opt.pc_feat_channels['pc_vx'] # = 1
                    flipped[0, channel, :, :] *= -1
                    pc_dep = torch.cat((pc_dep, flipped), axis=0)
                #  now pc_dep concat two mask images, left right flip with each other . 
        ...
        for scale in self.opt.test_scales:
            ...
            if not pre_processed:
                ...
                if 'pre_dets' in pre_processed_images['meta']: # not in
                    meta['pre_dets'] = pre_processed_images['meta']['pre_dets']
                if 'cur_dets' in pre_processed_images['meta']: # not in
            ...
            # reformat dets, transform outputs to ori-image coord
        ...
        # filter detection results by score thre and positive bbox
    ...
    def process(self, images, pre_images=None, pre_hms=None,
        ...
        with torch.no_grad():
            ...
            output = self.model(images, pc_dep=pc_dep, calib=calib)[-1]
                ...
                # {"x":images, "pc_dep":pc_dep, "calib":calib}
            # TODO 1. about hm and    dep sigmoid decode 
            ...
            # TODO 2. decode 2d-bboxs, filter out topK attributes based on score.
    ...
    def debug(self, debugger, images, dets, output, scale=1, 
        ...
        # TODO : debugger.add_blend_img , input background img and foreground img, save the blend img to debugger.imgs
        ...
        # pre_images is None if    self.opt.tracking
...
################################################################
    def _get_data(self, images, pre_images=None, pre_hms=None,
        pre_inds=None, return_time=False, pc_dep=None, meta=None):
        with torch.no_grad():
            calib = torch.from_numpy(meta['calib']).float().to(images.device).squeeze(0)
            torch.cuda.synchronize()
            return images[0],pc_dep[0],calib

    def  get_data(self, image_or_path_or_tensor, meta={}):
        load_time, pre_time, net_time, dec_time, post_time = 0, 0, 0, 0, 0
        merge_time, track_time, tot_time, display_time = 0, 0, 0, 0
        self.debugger.clear()
        start_time = time.time()
