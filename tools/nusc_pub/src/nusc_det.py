...
rp = "/home/wanghao/Desktop/projects/CenterPoint/det_results/det_results/"


def get_dets_fname_wh(frame_name, prefix=rp, thr=THRESH):
...
def get_dets_fname(frame_name, thres=THRESH):
...
if  __name__ == "__main__":
    ...
    LIDAR_PATH = "/home/wanghao/Desktop/projects/CenterPoint/det_results/nusc_voxelnet_0075_epoch20/"

    ANNO_PATH = "/mnt/data/waymo_opensets/val/annos/seq_%s_frame_"%SEQ_IDX

    lidar_ps = glob.glob(LIDAR_PATH+"*npy")
    lidar_ps.sort()
    frame_nums = len(lidar_ps)
    print(frame_nums)
    ...
    while not rospy.is_shutdown():
        ...
        lidar_path = lidar_ps[frame]
        lidar_name = os.path.basename(lidar_path)
        point_cloud = np.load(lidar_path)[:,1:-1]
        
        # box3d_preds, label_preds, scores = get_dets_fname_wh("seq_%s_frame_%s"%(SEQ_IDX, frame))
        box3d_preds, label_preds, scores = get_dets_fname( rp + lidar_name.replace("npy",'pkl') )
        ...
        if TRACK : 
            ...
            # yaw_transform = []
            # for ii in range(n_):
            #     mat_mul = vel_to_global[:3,:3].dot(rot_z[ii])
            #     yaw_ = np.arctan2(mat_mul[1,0], mat_mul[1,1])
            #     yaw_transform.append(yaw_)
            
            box3d_preds = np.concatenate([box3d_preds,scores.reshape(-1,1), label_preds.reshape(-1,1)],axis = -1)
            box3d_preds = mot_tracker.update(box3d_preds)
        ...
        if TRACK:
            ...
            box3d_preds[:,:3] = box3d_preds[:,:3].dot(vel_to_global_inv[:3,:3].T) + vel_to_global_inv[:3,-1]
            box3d_preds[:,6] -= rpy[2]
        ...
        if TRACK :
            scores = np.around (box3d_preds[:,-3], 3)
