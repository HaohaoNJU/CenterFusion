    ...
    def _display_3dbox_on_image(self, img, corners8, color = [255, 0, 255], all_in = True, show = True, format_2d = None,  text = None):
        ...
        if format_2d:
            ...
            if text is not None:
                ...
                cv2.putText(img, text =  str(text), org = place,  fontFace = cv2.FONT_HERSHEY_COMPLEX, fontScale = 1.0, color = color, thickness = 1)
        elif show:
            # Draw a circle for each point.
            ...
            if text is not None:
                place = (int(corners8_proj[:,0].min()), int(corners8_proj[:,1].min())-5)
                cv2.putText(img, text =  str(text), org = place,  fontFace = cv2.FONT_HERSHEY_COMPLEX, fontScale = 1.0, color = color, thickness = 1)
